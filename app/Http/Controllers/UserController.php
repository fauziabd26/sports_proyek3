...
class UserController extends Controller
{
    ...
    public function login_member(Request $request)
    {
        ...
        } elseif (Auth::attempt(array('username' => $username, 'password' => $password, 'isactive' => '1', 'role' => 'calonadmin'), false)) {
            return redirect()->route('mitra');
        } elseif (Auth::attempt(array('username' => $username, 'password' => $password, 'isactive' => '1', 'role' => 'superadmin'), false)) {
            return redirect()->route('user.index');
        } elseif (Auth::attempt(array('username' => $username, 'password' => $password, 'isactive' => '1', 'role' => 'admin'), false)) {
            return redirect()->route('mitra');
        } else {
            return redirect()->back();
        }
    ...
    public function adminstore(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $role = Role::where('name', "calonadmin")->first();
            $user->roles()->attach($role->id);
            Auth::attempt(array('username' => $user->username, 'password' => $request->password, 'isactive' => 1, 'role' => 'calonadmin'));
            $data['response_status'] = 1;
            $data['response_message'] = 'User was successfully created';
            return redirect()->route('mitra');
        }
